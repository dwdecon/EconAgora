...
export default async function ManifestoSection() {
  ...
  const metrics = [
    { value: `${prompts.count ?? 0}+`, label: locale === "zh" ? "提示词模板" : "Prompt templates" },
    { value: `${skills.count ?? 0}+`, label: locale === "zh" ? "可复用技能" : "Reusable Skills" },
  ];
  ...
  return (
    ...
    >
      ...
      <Reveal direction="up" threshold={0.3} className="max-w-[640px]">
        ...
        >
          <span className="pr-3">{content.nav.register}</span>
